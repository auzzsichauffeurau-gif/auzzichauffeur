...
export default function LeadsPage() {
    ...
    const handleSendQuote = async () => {
        ...
        const loadingToast = toast.loading(`Sending quote to ${selectedLead.customer_email}...`);

        try {
            const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://auzziechauffeur.com.au';
            const acceptUrl = `${siteUrl}/accept-quote/${selectedLead.id}`;
            ...
            const res = await fetch('/api/send-email', {
                ...
                body: JSON.stringify({
                    ...
                    to: selectedLead.customer_email,
                    subject: `Your Chauffeur Quote — Auzzie Chauffeur`,
                    html: `
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
                            <h2 style="color: #1e3a8a;">Your Quote from Auzzie Chauffeur</h2>
                            <p>Dear ${selectedLead.customer_name},</p>
                            <p>Thank you for your enquiry. Here are the details of your quote:</p>
                            <div style="background-color: #f3f4f6; padding: 15px; border-radius: 8px; margin: 20px 0;">
                                <p style="margin: 6px 0;"><strong>Pickup Date:</strong> ${selectedLead.pickup_date}</p>
                                <p style="margin: 6px 0;"><strong>Pickup Time:</strong> ${selectedLead.pickup_time}</p>
                                <p style="margin: 6px 0;"><strong>From:</strong> ${selectedLead.pickup_location}</p>
                                <p style="margin: 6px 0;"><strong>To:</strong> ${selectedLead.dropoff_location}</p>
                                <p style="margin: 6px 0;"><strong>Vehicle:</strong> ${selectedLead.vehicle_type}</p>
                                ${selectedLead.amount ? `<p style="margin: 6px 0;"><strong>Estimated Price:</strong> $${selectedLead.amount}</p>` : ''}
                            </div>
            ...
            if (!res.ok) {
                ...
                throw new Error(err.details || 'Email failed');
            }
            ...
            toast.error('Failed to send quote: ' + e.message, { id: loadingToast });
        }
    };
