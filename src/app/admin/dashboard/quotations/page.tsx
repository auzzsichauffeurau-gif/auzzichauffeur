    ...
    const handleSendQuote = async () => {
        ...
        try {
            ...
            const acceptButton = `
                <div style="margin: 30px 0; text-align: center;">
                    ...
                    <p style="margin-top: 10px; font-size: 0.8rem; color: #9ca3af;">
                        Clicking this button confirms your booking
                    </p>
                </div>
            `;
            ...
            await fetch('/api/send-email', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    from: 'Auzzie Chauffeur System <booking@auzziechauffeur.com.au>',
                    to: 'info@auzziechauffeur.com.au',
                    subject: `Quote Sent: ${selectedQuote.customer_name}`,
                    html: `
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
                            <h2 style="color: #1e3a8a; border-bottom: 2px solid #e5e7eb; padding-bottom: 0.5rem;">Quote Sent Notification</h2>
                            <p>A quote has been sent to the following customer:</p>
                            <div style="background-color: #f3f4f6; padding: 15px; border-radius: 8px; margin: 16px 0;">
                                <p style="margin: 6px 0;"><strong>Name:</strong> ${selectedQuote.customer_name}</p>
                                <p style="margin: 6px 0;"><strong>Email:</strong> ${selectedQuote.customer_email}</p>
                                <p style="margin: 6px 0;"><strong>Phone:</strong> ${selectedQuote.customer_phone}</p>
                            </div>
                            <div style="background-color: #eff6ff; padding: 15px; border-radius: 8px; margin: 16px 0;">
                                <p style="margin: 6px 0;"><strong>Pickup:</strong> ${selectedQuote.pickup_date} at ${selectedQuote.pickup_time}</p>
                                <p style="margin: 6px 0;"><strong>From:</strong> ${selectedQuote.pickup_location}</p>
                                <p style="margin: 6px 0;"><strong>To:</strong> ${selectedQuote.dropoff_location}</p>
                                <p style="margin: 6px 0;"><strong>Vehicle:</strong> ${selectedQuote.vehicle_type}</p>
                                <p style="margin: 6px 0;"><strong>Quoted Amount:</strong> $${selectedQuote.amount || '—'}</p>
                            </div>
                            <p style="color: #6b7280; font-size: 0.85rem;">Quote email subject: <em>${emailSubject}</em></p>
                            <p style="color: #6b7280; font-size: 0.85rem;">A follow-up task has been auto-scheduled for 2 days from now.</p>
                        </div>
                    `
                })
            });
            ...
            // 4. Create Follow-up Task AUTOMATICALLY
